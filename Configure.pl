...
if ($args{'show-autovect-failed'}) {
    ...
    push @cflags, ("-ftree-vectorizer-verbose=$args{'show-autovect-failed'}", "-fopt-info-vec-missed") if $config{cc} eq 'gcc';
}
...
Prints debug messages (hopefully) revealing why autovectorization has failed
for a loop. Verbosity level is 1-3 for clang, for GCC it is likely 1-2.
If you are trying to vectorize code, it's *highly* recommended start using clang
first as it's much smarter and has more useful messages. Then once it is working,
